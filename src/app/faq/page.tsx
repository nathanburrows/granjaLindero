"use client";

import { useState } from "react";
import { LangProvider, useLang } from "@/lib/LangContext";
import { t } from "@/lib/translations";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { waUrl } from "@/lib/contact";

function ChevronIcon({ open }: { open: boolean }) {
  return (
    <svg
      className={`w-5 h-5 text-stone-400 transition-transform duration-300 flex-shrink-0 ${open ? "rotate-180" : ""}`}
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
    >
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
    </svg>
  );
}

function FaqContent() {
  const { lang } = useLang();
  const tx = t[lang].faq;
  const [open, setOpen] = useState<number | null>(null);

  return (
    <main className="min-h-screen bg-stone-50">
      {/* Hero banner */}
      <div className="bg-stone-900 pt-32 pb-16 px-6 text-center">
        <span className="inline-block text-green-400 text-xs font-bold uppercase tracking-[0.2em] mb-4">
          {tx.label}
        </span>
        <h1 className="font-serif text-white text-4xl md:text-5xl font-bold leading-tight mb-4">
          {tx.title}
        </h1>
        <p className="text-stone-400 text-lg max-w-xl mx-auto">{tx.subtitle}</p>
      </div>

      {/* FAQ list */}
      <div className="max-w-3xl mx-auto px-6 py-16">
        <div className="space-y-3">
          {tx.items.map((item, i) => (
            <div
              key={i}
              className="bg-white rounded-2xl border border-stone-200 overflow-hidden"
            >
              <button
                onClick={() => setOpen(open === i ? null : i)}
                className="w-full flex items-center justify-between gap-4 px-6 py-5 text-left"
              >
                <span className="font-semibold text-stone-900 text-base">{item.q}</span>
                <ChevronIcon open={open === i} />
              </button>
              {open === i && (
                <div className="px-6 pb-5 text-stone-600 text-sm leading-relaxed border-t border-stone-100 pt-4">
                  {item.a}
                </div>
              )}
            </div>
          ))}
        </div>

        {/* WhatsApp CTA */}
        <div className="mt-14 text-center bg-green-50 rounded-3xl p-10">
          <p className="text-stone-700 font-semibold text-lg mb-2">
            {lang === "es" ? "¿No encontraste tu respuesta?" : "Didn't find your answer?"}
          </p>
          <p className="text-stone-500 text-sm mb-6">
            {lang === "es"
              ? "Escríbenos directamente y te respondemos en minutos."
              : "Message us directly and we'll reply in minutes."}
          </p>
          <a
            href={waUrl("")}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white font-semibold px-7 py-3.5 rounded-full transition-colors"
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
            WhatsApp
          </a>
        </div>
      </div>
    </main>
  );
}

export default function FaqPage() {
  return (
    <LangProvider>
      <Navbar />
      <FaqContent />
      <Footer />
    </LangProvider>
  );
}
