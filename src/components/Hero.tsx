...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      <video
        ref={vidA}
        className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-[7000ms] ${aOnTop ? "opacity-100 z-0" : "opacity-0 -z-10"}`}
      ...
      <video
        ref={vidB}
        className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-[7000ms] ${aOnTop ? "opacity-0 -z-10" : "opacity-100 z-0"}`}
        src="/video/hero-bg.mp4"
        muted
        playsInline
      />
