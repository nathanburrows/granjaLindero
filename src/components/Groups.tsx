...
function GroupModal({ onClose, lang }: { onClose: () => void; lang: "es" | "en" }) {
    ...
    >
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="flex items-center justify-between px-7 pt-7 pb-5 border-b border-stone-100">
          <div>
            <h3 className="font-serif text-stone-900 text-2xl font-bold">{tx.formTitle}</h3>
            <p className="text-stone-400 text-sm mt-0.5">{tx.formSubtitle}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-7 py-6 space-y-5">
          {/* Date */}
          <div>
            <label className="block text-sm font-semibold text-stone-700 mb-2">{tx.formDate}</label>
            <input
              type="date"
              value={date}
              onChange={(e) => setDate(e.target.value)}
              min={new Date().toISOString().split("T")[0]}
              className="w-full border border-stone-200 text-stone-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all"
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-semibold text-stone-700 mb-2">
              {tx.formPeople}: <span className="text-green-600 font-bold">{people}</span>
            </label>
            <input
              type="range"
              min={5}
          ...
          {/* Interests */}
          <div>
            <label className="block text-sm font-semibold text-stone-700 mb-3">{tx.formInterest}</label>
          ...
          <a
            ...
            className="flex items-center justify-center gap-3 w-full bg-green-600 hover:bg-green-500 text-white font-semibold text-base px-8 py-4 rounded-2xl transition-colors shadow-md"
          >
            <WhatsAppIcon />
            {tx.formSend}
          </a>
...
export default function Groups() {
  ...
  return (
    <section id="grupos" className="bg-stone-950 text-white py-24 px-6">
        ...
        <div className="grid md:grid-cols-2 gap-10 mb-16">
          <div className="bg-stone-900 rounded-2xl border border-stone-800 p-8">
            ...
            <div className="grid grid-cols-2 gap-3">
              {tx.activities.map((a) => (
                <div key={a.label} className="flex items-center bg-stone-800/60 rounded-xl px-4 py-3">
          ...
          <div className="bg-stone-900 rounded-2xl border border-stone-800 p-8">
            ...
            <div className="grid grid-cols-2 gap-3">
              {tx.groupTypes.map((g) => (
                <div key={g.label} className="flex items-center bg-stone-800/60 rounded-xl px-4 py-3">
                  <span className="text-stone-200 text-sm font-medium">{g.label}</span>
                </div>
              ))}
            </div>
            <p className="mt-6 text-stone-500 text-sm border-t border-stone-800 pt-4">{tx.note}</p>
          </div>
        </div>
      ...
      {showModal && <GroupModal onClose={() => setShowModal(false)} lang={lang} />}
    </section>
  );
}
